    ...
    <div className="min-h-screen flex flex-col bg-[#f8f9fa] relative overflow-x-hidden">
      ...
      <header className="fixed w-full top-0 z-40 glass-panel border-b border-white/50 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-18 sm:h-20 flex items-center justify-between">
          <div className="flex items-center gap-3 cursor-pointer group" onClick={() => window.location.hash = user ? '#/dashboard' : '#/'}>
          ...
          <nav>
            {user ? (
              <div className="flex items-center gap-6">
                ...
                <Button variant="secondary" onClick={signOut} className="!py-2 !px-4 !text-sm !rounded-lg border-ink-200">Sign Out</Button>
              </div>
            ) : (
              <div className="flex items-center gap-4">
                 <a href="#/login" className="text-sm font-semibold text-ink-600 hover:text-brand-600 transition-colors">Log in</a>
                 <Button variant="primary" className="!py-2.5 !px-5 !text-sm shadow-brand-500/20" onClick={() => window.location.hash = '#/signup'}>Get Started</Button>
              </div>
            )}
          </nav>
        </div>
